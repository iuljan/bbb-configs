...
read -rp "Do you want to change the password? [yes/no]: " change_pass
if [[ "$change_pass" =~ ^[Yy]([Ee][Ss])?$ ]]; then
    read -rp "Enter the password file (default: wifi_pass): " pass_file
    pass_file=${pass_file:-wifi_pass}
    read -rp "Enter new password: " new_password
...
for FILE_PATH in "${SORTED_FILES[@]}"; do
    ...
    if check_reachability "$HOSTNAME"; then
	    ...
	    # Update password file on the device before SCP
	    if [[ "$change_pass" =~ ^[Yy]([Ee][Ss])?$ ]]; then
		echo "$new_password" | ssh -q -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o BatchMode=yes "root@$HOSTNAME" "cat > /root/$pass_file"
		echo "Password has been updated on $HOSTNAME:/root/$pass_file."
	    fi
